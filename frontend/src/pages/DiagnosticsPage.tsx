...
export function DiagnosticsPage() {
  ...
  useEffect(() => {
    const writer = HanziWriter.create("hanzi-target", "學", { width: 180, height: 180, showOutline: true, strokeAnimationSpeed: 1 });
    writerRef.current = writer;
    setResults((current) => ({ ...current, "Hanzi Writer": { status: "PASS", detail: "學 rendered" } }));
  ...
  async function record() {
    try {
      ...
      recorder.onstop = () => { stream.getTracks().forEach((track) => track.stop()); setRecording(`PASS：${new Blob(chunks).size} bytes，僅暫存於記憶體`); };
