...
export function LearningPage({ activeChildId }: { activeChildId: number | null }) {
  ...
  const [readingActivityDomain, setReadingActivityDomain] = useState<"speaking" | "pronunciation">("speaking");
  ...
  async function speak(text: string, textKind: TTSTextKind, locale = ttsLocale, curriculumItemId?: string) {
    try {
      const payload = await api<any>("/api/tts/speak", { method: "POST", body: JSON.stringify({ text, text_kind: textKind, locale, rate: ttsRate }) });
      let listeningAttemptId: string | null = null;
      if (childId && curriculumItemId) {
        try { listeningAttemptId = (await api<any>(`/api/children/${childId}/listening-attempts`, { method: "POST", body: JSON.stringify({ item_id: curriculumItemId }) })).id; } catch { /* Unlinked practice remains playback-only. */ }
      }
      const startedAt = Date.now();
      ttsProvider.speak(payload, {
        onEnd: () => { if (childId && listeningAttemptId) void api(`/api/children/${childId}/listening-attempts/${listeningAttemptId}/complete`, { method: "POST", body: JSON.stringify({ duration_ms: Date.now() - startedAt }) }).catch(() => undefined); },
        onError: () => { if (childId && listeningAttemptId) void api(`/api/children/${childId}/listening-attempts/${listeningAttemptId}/abort`, { method: "POST", body: "{}" }).catch(() => undefined); },
      });
      setMessage(`TTS played (${locale}, ${ttsRate}x)`);
    } catch (error) { setMessage(error instanceof Error ? error.message : "TTS unavailable"); }
  }
  async function startReadingAloud() {
    if (!childId || !readingText.trim()) return;
    try {
      const attempt = await api<any>(`/api/reading-aloud/attempts/start?child_id=${childId}`, { method: "POST", body: JSON.stringify({ text: readingText, text_kind: readingKind, locale: readingLocale, source_type: readingSourceType, source_id: readingSourceId, activity_domain: readingActivityDomain }) });
  ...
  return <main className="practice-content" aria-label="Chinese practice activities"><header><p className="eyebrow">Practice</p><h1>TongXuan Chinese</h1><p>Choose a short activity to practice Chinese.</p></header>
    <section className="card"><h2>認字小挑戰 · Character challenge</h2><button onClick={start} disabled={!childId}>Start a quick round</button>{item && <div><h3>認一認：{item.character}</h3><button onClick={() => speak(item.character, "character", "zh-TW", item.id)}>🔊 Listen</button><button onClick={() => answer("correct")}>Correct</button><button onClick={() => answer("incorrect")}>Incorrect</button><button onClick={() => answer("correct", true)}>Hint / assisted</button></div>}</section>
    ...
    <section className="card"><h2>更多中文練習</h2><button onClick={seedB}>Load practice activities</button>{sprintB && <div>
      <h3>Words + Sentences</h3><p>{sprintB.words.length} words / {sprintB.sentences.length} sentences</p>{sprintB.words.map((word: any) => <span key={word.id}><button onClick={() => practiceWord(word.id)}>Practice {word.word}</button><button onClick={() => speak(word.word, "word", ttsLocale, word.id)}>🔊</button></span>)}{sprintB.sentences.map((sentence: any) => <span key={sentence.id}><button onClick={() => practiceSentence(sentence)}>Practice sentence</button><button onClick={() => speak(sentence.sentence, "sentence", ttsLocale, sentence.id)}>🔊</button></span>)}
