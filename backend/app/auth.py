...
def issue_session(*, subject: str, role: str, ttl_seconds: int = 3600) -> str:
    """Provision a token for an external auth adapter or tests; never an API route."""
    if role not in ALLOWED_ROLES:
        raise ValueError("invalid_role")
    payload = {"subject": subject, "role": role, "exp": int(time.time()) + ttl_seconds}
...
def authenticate(request: Request) -> Session:
    ...
    try:
        ...
        session = Session(str(payload["subject"]), str(payload["role"]), int(payload["exp"]))
