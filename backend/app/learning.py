    ...
    with connect() as db:
        ...
        row = db.execute(
            """SELECT i.id, i.character, i.curriculum_source, COALESCE(s.correct_count,0) correct_count,
               ...
               WHERE i.child_id=? ORDER BY COALESCE(s.due_at, CURRENT_TIMESTAMP), i.id LIMIT 1""", (child_id,)
        ).fetchone()
...
def finish_session(child_id: int, session_id: str) -> dict[str, Any]:
    with connect() as db:
        ...
        count = db.execute("SELECT COUNT(*) n FROM recognition_attempts WHERE session_id=?", (session_id,)).fetchone()["n"]
...
def create_weekly_test(child_id: int) -> dict[str, Any]:
    with connect() as db:
        ensure_child(db, child_id)
        rows = db.execute("SELECT id, character FROM learning_items WHERE child_id=? ORDER BY id LIMIT 5", (child_id,)).fetchall()
        test_id = uid("test")
